...
fig, axs = plt.subplots(5, 1, figsize = (2.0*3.3, 2.0*3.6*2.04), sharex = True, sharey = True)

spec_ch0s_all = []

for a, spec_file_set, noisefile, DP0, DALPHA in zip(axs[:-1], spec_file_sets, noisefiles, DP0s, DALPHAs):
    print(len(spec_file_set))
    
    print(noisefile)
    gw_log10_A = np.log10(2.2E-15)#noisefile['gw_log10_A']
    gw_gamma = 13.0/3.0 #noisefile['gw_gamma']
...
#fig, (ax1) = plt.subplots(1,1, figsize = (6,4))# ,figsize=(10,7))#, xscale = 'log', yscale = 'log')
        ...
        freq = spec[:, 0]/86400.0
        psd = ((spec[:, 1])*(u.year)**3.0).to((u.s)**3.0).value#spec[:, 1]*((1.0*u.year).to(u.s).value)**3.0
    ...
    a.text(9E-8, 1E-2, r'$\Delta \log_{{10}} P_0 = {:.1f}$, $\Delta \gamma = {:.1f}$'.format(DP0, DALPHA), fontsize = 12, horizontalalignment = 'right')
    #a.text(
    a.set_xscale('log')
    a.set_yscale('log')
    # if DALPHA == DALPHAs[-1]:##a == axs[-1]:
    #     a.set_xlabel(r'Frequency$\,$(Hz)', fontdict=font)
    a.set_ylabel(r'$\mathcal{P}\,$(s$^{{3}}$)', fontdict = font)
    a.tick_params(axis='y', labelsize = tickfont['size'])
    a.tick_params(axis='x', labelsize = tickfont['size'])
    
    a.set_xlim(1E-9,1E-7)

spec_dir_fmt = '../pptadr2_spec/'

spec_dirs = [spec_dir_fmt]

spec_file_sets = [sorted(glob.glob('{}/*.spec'.format(dir))) for dir in spec_dirs]

for a, spec_file_set in zip([axs[-1]], spec_file_sets):#, DP0s, DALPHAs):
    print(len(spec_file_set))
    gw_log10_A = np.log10(2.2E-15)#noisefile['gw_log10_A']
    gw_gamma = 13.0/3.0 #noisefile['gw_gamma']
    for spec_files in spec_file_set:
...
#fig, (ax1) = plt.subplots(1,1, figsize = (6,4))# ,figsize=(10,7))#, xscale = 'log', yscale = 'log')
        ...
        spec = np.loadtxt(spec_files)
        freq = spec[:, 0]/86400.0
        psd = ((spec[:, 1])*(u.year)**3.0).to((u.s)**3.0).value
        spec_ch0s.append(psd[0])
        if spec_files == spec_files[0]:
            label = os.path.splitext(os.path.basename(spec_files))[0]
        else:
        ...
        a.plot(freq, psd, label = label, color = 'k', linewidth = 0.8, alpha = 0.5) #9E9E9E
    ...
    a.set_xscale('log')
    a.set_yscale('log')
    a.text(9E-8, 1E-2, r'PPTA DR2', fontsize = 12, horizontalalignment = 'right')
    a.set_xlabel(r'Frequency$\,$(Hz)', fontdict=font)
    a.set_ylabel(r'$\mathcal{P}\,$(s$^{{3}}$)', fontdict = font)
    a.tick_params(axis='y', labelsize = tickfont['size'])
    a.tick_params(axis='x', labelsize = tickfont['size'])#font['size'])
