...
noise_samps = ['5.60_0.00', '7.00_0.00', '5.60_4.00', '7.00_4.00', '8.40_5.60']
...
fig, axs = plt.subplots(6, 1, figsize = (3.3, 3.6*2.04), sharex = True, sharey = True)

spec_ch0s_all = []

for a, spec_file_set, DP0, DALPHA in zip(axs[:-1], spec_file_sets, DP0s, DALPHAs):
...
#fig, (ax1) = plt.subplots(1,1, figsize = (6,4))# ,figsize=(10,7))#, xscale = 'log', yscale = 'log')
    ...
    a.text(9E-8, 1E-2, r'$\Delta \log_{{10}} P_0 = {:.1f}$, $\Delta \alpha = {:.1f}$'.format(DP0, DALPHA), fontsize = 10, horizontalalignment = 'right')
    #a.text(
    a.set_xscale('log')
    a.set_yscale('log')
...
#fig, (ax1) = plt.subplots(1,1, figsize = (6,4))# ,figsize=(10,7))#, xscale = 'log', yscale = 'log')
    ...
    a.set_xscale('log')
    a.set_yscale('log')
    a.text(9E-8, 1E-2, r'PPTA DR2', fontsize = 10, horizontalalignment = 'right')
    a.set_xlabel(r'Frequency$\,$(Hz)', fontdict=font)
    a.set_ylabel(r'$\mathcal{P}\,$(s$^{{3}}$)', fontdict = font)
    a.tick_params(axis='y', labelsize = 12)
    a.tick_params(axis='x', labelsize = 12)#font['size'])
...
fig.tight_layout()
plt.savefig('_cholspec_range_fit_thresh.png', dpi = 300, bbox_inches = 'tight', facecolor='white')
plt.savefig('_cholspec_range_fit_thresh.pdf',  bbox_inches = 'tight', facecolor='white')
#plt.show()
