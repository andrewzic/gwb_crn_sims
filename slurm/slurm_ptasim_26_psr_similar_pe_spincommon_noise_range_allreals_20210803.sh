...
#SBATCH --output=/scratch1/zic006/gwb_crn_sims/slurm_logs/pe_spincommon__20210803_%A_%a.log
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --nodes=1
#SBATCH --time=0-05:00:00
#SBATCH --mem=4G
#SBATCH --array=8001-11999
...
#/scratch1/zic006/gwb_crn_sims/params/params_all_pe_array_spincommon_0.00_0.00_20210803_r0.dat
paramfiles=($(ls -v /scratch1/zic006/gwb_crn_sims/params/all_pe_array_spincommon/params_all_pe_array_spincommon_*_r[0-9]*.dat))
IFS=$'\n' paramfiles=($(sort -k1.88r <<<"${paramfiles[*]}")); unset IFS
...
outdir=${outdir}/
datadir=`grep "datadir: " ${PRFILE} | awk '{print $2}'`
datadir=${datadir}
datadir_trim=$( echo ${datadir} | sed 's:/*$::' )
datadir_tar=${datadir_trim}.tar.gz
...
echo "UNPACKING ${datadir_tar}"
#the /../../ is because the tarballs are structured like regsamp_0.00_0.00/output/real_0/
tar -xvf ${datadir_tar} -C $( dirname ${datadir_tar} )/../../

if [ ! -d ${datadir} ]
then
    echo "${datadir} NOT FOUND. EXITING"
    exit 2
fi


#IF ( THE OPTIMAL STATISTIC RESULTS AREN't THERE ) AND ( EITHER THE CHAIN ISN'T THERE OR THE CHAIN IS NOT BIG ENOUGH )
if [ ! -f "${os_pickle_file}" ] && [[ ! -f "${chain_file}"  ||  (( ${nlines_chain} < "${chain_minlines}" )) ]]
then
    
    singularity exec /home/zic006/psr_gwb.sif python3 /scratch1/zic006/gwb_crn_sims/run_enterprise_simple.py --prfile ${PRFILE}
	...
	bad_prfile=$(echo ${PRFILE} | sed -r 's|params/all_pe_array_spincommon/all|params/all_pe_array_spincommon/bad|g')
	cp ${PRFILE} ${bad_prfile}
	
	singularity exec /home/zic006/psr_gwb.sif python3 /scratch1/zic006/gwb_crn_sims/run_enterprise_simple.py --prfile ${bad_prfile}
	...
	singularity exec /home/zic006/psr_gwb.sif python3 -m enterprise_warp.results --result ${PRFILE} --info 1 -c 2 -p "gw" -f 1 -l 1 -m 1 > /scratch1/zic006/gwb_crn_sims/result_logs/`basename ${PRFILE} .dat`.result
...
#/scratch1/zic006/gwb_crn_sims/tidy_result.sh $( realpath ${outdir} )

#CLEAN UP
#if [ -f ${outdir}/OS_SUCCESS ] && [ -f ${dirname}/_os_results.pkl ]
if [ -f ${outdir}/CHAIN_SUCCESS ] && [ -d ${dirname}/noisefiles/ ]
then
	...
	#if [ -f ${PRFILE} ];
	#then
	#cp ${PRFILE} $(echo ${PRFILE} | sed -r 's|params/all_pe_array_spincommon/all|params/all_pe_array_spincommon/done|g')
	#fi
    fi
fi
...
then
    if [ -f ${datadir_tar} ]
    then
	echo "${datadir_tar} EXISTS. REMOVING DATA DIRECTORY ${datadir}"
	rm -r ${datadir}
    fi
fi
...
#grep -Fxq "Run complete with 11000 effective samples" /scratch1/zic006/gwb_crn_sims/slurm_logs/gwb_crn_sims_pe_spincommon_noise_range_20210803_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.log;

	# for psr in `cat /scratch1/zic006/gwb_crn_sims/psrs.dat`; do 
	#     echo $psr;
