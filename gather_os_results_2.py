...
corr_os_matrix = {'hd': np.nan * np.zeros_like(os_matrix),
                  'dipole': np.nan * np.zeros_like(os_matrix),
                  'monopole': np.nan * np.zeros_like(os_matrix)
                  }

corr_os_marg_matrix = {'hd': np.nan * np.zeros_like(os_matrix),
                       'dipole': np.nan * np.zeros_like(os_matrix),
                       'monopole': np.nan * np.zeros_like(os_matrix)
                       }

corr_sqrtos_matrix = {'hd': np.nan * np.zeros_like(os_matrix),
                      'dipole': np.nan * np.zeros_like(os_matrix),
                      'monopole': np.nan * np.zeros_like(os_matrix)
}

corr_sqrtos_marg_matrix = {'hd': np.nan * np.zeros_like(os_matrix),
                           'dipole': np.nan * np.zeros_like(os_matrix),
                           'monopole': np.nan * np.zeros_like(os_matrix)
}

corr_os_snr_matrix = {'hd': np.nan * np.zeros_like(os_matrix),
                      'dipole': np.nan * np.zeros_like(os_matrix),
                      'monopole': np.nan * np.zeros_like(os_matrix)
                      }
corr_os_marg_snr_matrix = {'hd': np.nan * np.zeros_like(os_matrix),
                           'dipole': np.nan * np.zeros_like(os_matrix),
                           'monopole': np.nan * np.zeros_like(os_matrix)
                           }
...
def plot_matrix(matrix, norm = None, type = 'os', measure = 'A', label = 'hd', dalphas = dalphas, dP0s = dP0s, close = True, cb_label = None, ax_title = None, vmin = None, vmax = None):

  measure_dict = {'A': r'$\hat{{A}}_{{\mathrm{{{}}}}}^{{2}}$', 'sA': r'$|\hat{{A}}_{{\mathrm{{{}}}}}|$', 'S/N': r'$\rho(\mathrm{{{}}})$', 'SNR': r'$\rho(\mathrm{{{}}})$', 'N': '$N_\mathrm{{{}}}$'}
  ...
  im = plt.imshow(np.abs(matrix), cmap = cmr.ocean_r, norm = norm, origin = 'lower', extent = [dalphas[0], dalphas[-1] + ddalphas, dP0s[0], dP0s[-1] + ddP0s], aspect = (8.0 + ddalphas)/(14.0 + ddP0s), interpolation = 'nearest', vmin = vmin, vmax = vmax)#, clim = [1E-6, 1E6]) #
  ...
  X,Y = np.meshgrid(dalphas+ddalphas*0/2.0, dP0s+ddP0s*0/2.0)
...
for corr in ['hd', 'dipole', 'monopole']:
  ...
  for os_result_all, dP0_ind, dalpha_ind, realisation_ind in zip(os_pickles, dP0_inds, dalpha_inds, result_realisations):
    if os_result_all == None:
        print('heck')
        corr_sqrtos_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.nan
        corr_sqrtos_marg_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.nan
        corr_os_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.nan
        corr_os_marg_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.nan
        corr_os_snr_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.nan 
        corr_os_marg_snr_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.nan
    else:
        os_result = os_result_all[corr]
        os_OS = os_result['OS']
        ...
        corr_os_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = os_OS
        corr_os_marg_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = mean_marg_OS
        corr_sqrtos_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.sqrt(np.abs(os_OS))
        corr_sqrtos_marg_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = np.sqrt(np.abs(mean_marg_OS))
        corr_os_snr_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = os_SNR
        corr_os_marg_snr_matrix[corr][dP0_ind, dalpha_ind, realisation_ind] = mean_marg_SNR
        
  pickle.dump(corr_os_matrix, open('corr_os_matrix.pkl', 'wb'))
  pickle.dump(corr_os_marg_matrix, open('corr_os_marg_matrix.pkl', 'wb'))
  pickle.dump(corr_sqrtos_matrix, open('corr_sqrtos_matrix.pkl', 'wb'))
  pickle.dump(corr_sqrtos_marg_matrix, open('corr_sqrtos_marg_matrix.pkl', 'wb'))
  pickle.dump(corr_os_snr_matrix, open('corr_os_snr_matrix.pkl', 'wb'))
  pickle.dump(corr_os_marg_snr_matrix, open('corr_os_marg_snr_matrix.pkl', 'wb'))
  ...
  plot_matrix(mean_corr_sqrtos_matrix[corr] - mean_corr_sqrtos_marg_matrix[corr], measure = 'sA', label = 'sqrt_mean_ML_marg{}'.format(corr_label), vmax = 8E-17)
  print('sqrt done')
  print('sqrt done')
...
# fig, (ax1, ax2) = plt.subplots(2, 1, figsize = (3.3, 2*2.04), sharex=True)
# labels = {'hd': 'HD', 'dipole': 'Dipole', 'monopole': 'Monopole'}

# for corr in ['hd', 'dipole', 'monopole']:
#     ax1.hist(corr_os_snr_matrix[corr].flatten(), label = labels[corr], density = True)
#     ax2.hist(corr_os_snr_marg_matrix[corr].flatten(), label = labels[corr], density = True)
