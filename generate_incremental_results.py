...
bf_matrix = 10.0**np.nanmedian(np.log10(np.array(bf_matrix_list)), axis = 0)
...
np.save('bf_matrix_vg.npy', bf_matrix)
np.save('bf_matrix_vg_all.npy', np.array(bf_matrix_list))
...
ax1.text(0.1, 0.85, s = r'$\langle \mathcal{{B}}^{{\mathrm{{CP1}}}}_{{\mathrm{{TN}}}} \rangle < 10^{{-2}}$', fontsize = font['size'], transform = ax1.transAxes)
ax2.text(0.1, 0.85, s = r'$10^{{-2}} < \langle \mathcal{{B}}^{{\mathrm{{CP1}}}}_{{\mathrm{{TN}}}} \rangle < 10^{{2}}$', fontsize = font['size'], transform = ax2.transAxes)
ax3.text(0.1, 0.85, s = r'$\langle \mathcal{{B}}^{{\mathrm{{CP1}}}}_{{\mathrm{{TN}}}} \rangle > 10^{{2}}$', fontsize = font['size'], transform = ax3.transAxes) 
x
#ax3.set_xlabel(r'$\mathcal{{B}}^{{\mathrm{{CP1}}}}_{{\mathrm{{TN}}}}$', fontsize = font['size'])
...
for ax in [ax1, ax2, ax3]:
    ...
    ax.set_xlim(-6,6)
    if ax == ax3:
        ax.set_xlabel(r'$\log_{{10}} \mathcal{{B}}^{{\mathrm{{CP1}}}}_{{\mathrm{{TN}}}}$', fontsize = font['size'])
...
fig.savefig('bf_matrix.pdf', bbox_inches = 'tight')
fig.savefig('bf_matrix.png', bbox_inches = 'tight', dpi = 300)
# plt.show()


bf_matrix = 10.0**np.nanmax(np.log10(np.array(bf_matrix_list)), axis = 0)
fig = plot_matrix(bf_matrix, dalphas, dP0s)
fig.savefig('max_bf_matrix.pdf', bbox_inches = 'tight')
fig.savefig('max_bf_matrix.png', bbox_inches = 'tight', dpi = 300)
